...
| Ruff 398、CI 红、无分支保护 | **仍成立（历史数字已过时），P1** | 当前 `uv run ruff check . --output-format concise` 为 **1 error**（`backend/domains/family/domain/entities.py:E501`）；prompt_registry 两项已在本轮 WIP 修复；`uv run pytest tests/architecture -q` 仍为 106 passed、1 skipped、4 failed；远端最新 CI run `33291355462` 仍有 I001；GitHub main branch protection 返回 404。历史“398”不再是当前计数。 | AQA/GOV/ARCH：修复 YAML、lint debt、登记漂移；CI 必跑并启用分支保护。 |
