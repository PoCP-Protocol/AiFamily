...
| S1（`VS-GROWTH-01` 真实闭环） | Growth + AFE/API + AAIR | 独立 `s01_vertical_slice.py`/后续 HTTP adapter、Journey application/HTTP、独占测试；不改 FGCN/Commerce writer | `b37b1b6`（已在远端历史，仍非生产完成）signal→Perspective/Hypothesis/Draft→家庭确认→GrowthIntent/ActionTask→Review，横跨 canonical S01+S04+S05+S07 | 正向家长流程；反向 AI actor/跨 tenant/撤回 consent/新 key replay/conflict/删除/断电重试/多 locale | HTTP+PG+outbox+audit+deletion/replay；UI-03→05→09 可回读；当前仍缺真实 PG/HTTP/构建/主线合入证据，保持 PARTIAL |
...
| FGCN / service collaboration | admission→Human Gate→Named Action→TaskAssignment→delivery/quality/contribution | FGCN / `41ad120`→`e7cbb0b` | Fresh `uv run pytest tests/domains/service/fgcn -q`（Postgres URL）113 passed；FGCN admission evidence/provenance 已绑定（仅在 `VS-GROWTH-01` 确认 need 后接入） | `GO (测试契约)/NO-GO (生产)` | reviewer/worker/action context 默认 RuntimeError；one-shot worker 无 queue/通知/DLQ；capacity reservation 非原子；gate/assignment 双事务 crash/retry；assignment 后续 COMPLETED/REVOKED 时同 request replay 可能误判 mismatch；场景校验硬编码英文、未接 locale registry；`family_request_ref`/失败次数缺 canonical 数据血缘与作用域验证；生产 identity/consent/duplicate operationId 待补 |
