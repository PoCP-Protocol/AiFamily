...
Implemented以外，都还不存在。当前诚实进度见第十九节。
```
...
## 五、四级运行对象（复述ADR-0167待决提案，标注本架构里的目标形态）
...
IntelligenceRun link → R2.6 Absorb Vertical Runtime`。ADR-0167当前仍为
`Proposed`，R2.1尚未完成决策冻结；R2.2及以后按ADR-0158记录的既有顺序，
需先核实R0 Green Main，再由总架构师批准启动。本文件不把提案升级为决定，
只提供获批后的技术路线图。
...
- **十二大核心子系统**：S05 Agent Runtime、S06 Model Gateway有真实实现
  ...
  已在既有记忆记录中确认）。
- **四级Run Taxonomy**：ADR-0167当前仍为`Proposed`，尚未完成架构语义
  冻结；`AgentRunPersistencePort`+`DurableAgentRuntime`是
