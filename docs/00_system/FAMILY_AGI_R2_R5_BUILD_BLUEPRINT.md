...
**状态：提案已形成，尚未完成**——ADR-0167当前仍为`Proposed`，四级Run
Taxonomy与`EvaluationLedger=DEPRECATED`仍是待接受的架构提案，不能描述为
已冻结裁决。除完成ADR决策程序外，还需补对应architecture test：
...
- **R2.1**：未完成。ADR-0167仍为`Proposed`，
  `tests/architecture/test_runtime_taxonomy.py`也尚未编写。
