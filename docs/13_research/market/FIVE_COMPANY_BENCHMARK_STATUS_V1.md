...
**决策记录（2026-09-07）**：project-owner 已就"品牌是否需要对外科学
背书"这一前提拍板——**需要**。已落地为 `ADR-0156`：区分"过程确定性
证据"（可披露：审核通过率、匹配-履约时延、Human Gate 拒绝率）与
"结果因果声明"（永久禁止：Maven 式"X 降低了 Y 结果"这类表述直接撞
R9 疗效红线）。当前真实交付样本量仍为零，**尚不满足发表门槛**，本次
只是把"允许做什么、禁止做什么"的边界写清楚，不是批准现在发表。触发
实际发表动作需要另一次产品决策确认样本量达标，详见 ADR-0156。
...
- **2026-09-07 复核**：网络环境（WebSearch/WebFetch）仍处于拦截状态，
  本次未能补上这条实时核验；上述置信度中等的结论维持原样，下次网络
  环境恢复后仍应优先补一次。

## 8. 后续更新（2026-09-07）

§3.2 的产品决策已由 project-owner 拍板并落地为 `governance/ADR/
ADR-0156-process-integrity-disclosure-not-efficacy-claim.md`，详见该节
决策记录。§6 的结论段"其余需要产品战略拍板的更大决策"现只剩 TikTok
式兴趣建模排课一项待定，Maven 式对外发表证据已转为受边界约束的
"未来可做，当前样本量不足"状态，不再是完全悬置。
