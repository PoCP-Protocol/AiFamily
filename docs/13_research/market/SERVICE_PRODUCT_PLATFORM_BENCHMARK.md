...
### 2.7 概念决策：先隔离理解，再并列比较

Maze 将概念测试区分为单概念隔离、多个概念依次隔离、直接比较，以及先隔离再比较的
protomonadic 方式；Dovetail 则强调研究结论应反链原始 highlight、note 或视频片段，并支持团队
共同修订。值得移植的是“先独立理解、再比较差异”和“摘要可下钻到原证据”，不是用一个总分
选出 AI 冠军。来源：[Maze concept testing](https://maze.co/guides/concept-testing/)、
[Dovetail insights](https://dovetail.com/product/insights/)（访问于 2026-08-31）。

对 AiFamily 的含义：候选必须保持输入顺序和等权展示；三区算法建议与人工批准并列；缺少六维
证据、存在关键反证或儿童安全问题时，系统生成 Return-to-Research DRAFT，而不是静默淘汰或
自动推进。新证据形成新的快照和修订，不能覆盖旧决策。
