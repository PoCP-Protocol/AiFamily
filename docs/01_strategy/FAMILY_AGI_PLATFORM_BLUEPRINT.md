...
> 能力描述，均以"目标态"书写；当前诚实进度见第三十八节。任何后续文档、汇报、代码
...
**与ADR-0167四级Run Taxonomy提案的对应关系**（当前仍为`Proposed`，未冻结）：
...
当前系统连Family World Model的最小雏形都未建成（见第三十八节），这一节是终局
判断依据，不是近期可兑现的能力声明。
...
| `governance/ADR/ADR-0167-family-agi-runtime-architecture.md` | 保留为待决策架构提案——四级Run Taxonomy是本蓝图第三节AGI内核的技术实现层候选定义，当前状态为`Proposed`，不得当作已冻结权威 |
...
**跟ADR-0167四级Run Taxonomy提案的映射**（不新增第五种runtime ledger；该ADR
仍为`Proposed`，以下是目标态约束而非已冻结决定）：`Reasoning & Planning Kernel`产生的模型调用是
