...
export type CommerceProjection = { family_id: string; projection_version: number; visibility: "FAMILY_PRIVATE"; order_intents: CommerceOrderIntent[]; entitlements: CommerceEntitlement[]; text_equivalent: string; read_only: true };
...
function validate(value: unknown): CommerceProjection {
  ...
  return { family_id: row.family_id, projection_version: row.projection_version as number, visibility: "FAMILY_PRIVATE", read_only: true,
    text_equivalent: row.text_equivalent, order_intents: row.order_intents as CommerceOrderIntent[], entitlements: row.entitlements as CommerceEntitlement[] };
}
