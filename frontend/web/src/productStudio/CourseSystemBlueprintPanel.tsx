...
export function CourseSystemBlueprintPanel({ client, deliveryClient, courseContentClient, courseContentId, tenantScope, enabled = true }: { client?: CourseSystemApiClient; deliveryClient?: CourseDeliveryProjectionApiClient; courseContentClient?: CourseContentReadApiClient; courseContentId?: string; tenantScope?: string; enabled?: boolean }) {
  ...
  useEffect(() => { if (!enabled) return; let active = true; void resolvedCourseContentClient.listPublished().then((courses) => { if (active) { setPublishedCourses(courses); setSelectedCourseContentId((current) => current || courses[0]?.id || ""); } }).catch(() => { if (active) setPublishedCourses([]); }); return () => { active = false; }; }, [enabled, resolvedCourseContentClient]);
