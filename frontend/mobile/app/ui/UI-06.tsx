...
export default function MyMembershipScreen() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((error) => {
        console.error("UI-06 remote action failed", error);
      });
    return () => {
      active = false;
    };
          ...
          <View>
            <View style={styles.topBar}>
              ...
              <View style={styles.topActions}>
                ...
                <Text style={styles.topCircle}>⊙</Text>
              </View>
            </View>
            ...
            <View style={styles.memberCard}>
              <View style={styles.memberHead}>
                <View style={styles.avatar}>
                  <IconSymbol name="person.crop.circle.fill" size={58} color="#09295A" />
                </View>
                <View style={styles.memberIdentity}>
                  <View style={styles.nameRow}>
                    <Text style={styles.memberName}>乐乐妈妈</Text>
                    <Text style={styles.memberBadge}>{summary.memberLabel}</Text>
                  </View>
            ...
            <Pressable onPress={() => router.push("/ui/UI-15" as Href)} style={({ pressed }) => [styles.inviteCard, pressed && styles.pressed]}>
              <View style={styles.inviteCopy}>
                ...
                <View style={styles.inviteTrack}>
                  <View style={[styles.inviteFill, { width: `${(invitedCount / 3) * 100}%` }]} />
                </View>
              </View>
              <View style={styles.inviteAction}>
                <Text style={styles.inviteActionText}>去邀请</Text>
              </View>
            </Pressable>
          </View>
        }
        renderItem={({ item }) => (
          <Pressable onPress={() => router.push(`/ui/${item.target}` as Href)} style={({ pressed }) => [styles.menuRow, pressed && styles.rowPressed]}>
            <View style={styles.menuIcon}>
              <IconSymbol name={item.icon} size={22} color={MENU_ICON_COLORS[item.id]} />
            </View>
          ...
          <Pressable onPress={() => router.push("/ui/UI-30" as Href)} style={({ pressed }) => [styles.annualCard, pressed && styles.pressed]}>
            <View style={styles.annualCopy}>
              ...
              <View style={styles.benefitButton}>
                <Text style={styles.benefitButtonText}>查看权益</Text>
              </View>
            </View>
            <View style={styles.crownPanel}>
              <IconSymbol name="crown.fill" size={70} color="#F4C75B" />
            </View>
          </Pressable>
        }
      />
...
const styles = StyleSheet.create({
  content: { paddingBottom: 34, backgroundColor: "#FFF9F3" },
  topBar: {
    minHeight: 67,
  ...
  memberStatLabel: { color: "#D8EEFF", fontSize: 12, lineHeight: 18 },
  memberStatValue: {
    color: "#FFFFFF",
    fontSize: 24,
    lineHeight: 30,
