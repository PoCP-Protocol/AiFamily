...
const BASELINE_COURSE_SECTIONS = [
  {
    title: "第1周：建立亲子沟通基础",
    items: ["理解孩子的心理需求", "掌握倾听的艺术", "建立信任关系"],
  },
  {
    title: "第2周：提升沟通技巧",
    items: ["情绪管理与表达", "冲突化解方法", "正向反馈技巧"],
  },
  {
    title: "第3周：巩固家庭关系",
    items: ["制定家庭规则", "培养孩子自主性", "持续成长计划"],
  },
] as const;
...
export default function GrowthProductDetailScreen() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((error) => {
        console.error("UI-14 remote action failed", error);
      });
    return () => {
      active = false;
    };
  ...
  const product = products.find((item) => item.productRef === productRef) ?? products.find((item) => item.productRef === state.commerceIntentDraft?.productRef) ?? products[0];
  ...
  const saveIntent = async () => {
    void PURCHASE_INTENT_BOUNDARY;
    const flowId = `commerce-intent:${product.productRef}`;
    state.setFlowStatus({
      flowId,
      lastAction: "SAVE_COMMERCE_INTENT_DRAFT",
    ...
    try {
      state.setFlowStatus({
        flowId,
        lastAction: "SUBMIT_COMMERCE_INTENT",
    ...
    <ScreenContainer edges={["left", "right", "bottom"]} containerClassName="bg-white" safeAreaClassName="bg-white" style={styles.screen}>
      ...
      <ScrollView refreshControl={<FamilyRefreshControl />} style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.topBar}>
          ...
          <View style={styles.capsule}>
            <Text style={styles.capsuleDots}>•••</Text>
        ...
        <View style={styles.priceBlock}>
          ...
          <Text style={styles.catalogBoundary}>当前为测试目录信息；保存只形成家庭私有意向，不会扣款。</Text>
          {showIntentState ? <Text style={styles.intentBadge}>已加入家庭权益意向</Text> : null}
        </View>

        <View style={styles.assuranceBar}>
          {(product.assurances ?? ["21天系统训练", "打卡社群陪伴", "专家顾问答疑"]).slice(0, 3).map((label) => (
            <AssurancePill key={label} label={label} />
          ))}
        </View>
          ...
          {product.delivery.slice(0, 4).map((item, index) => (
            <View key={`${product.productRef}-${item}`} style={styles.deliveryItem}>
              <View style={styles.deliveryIcon}>
                <Text style={styles.deliveryEmoji}>{BASELINE_DELIVERY_ICONS[index] ?? "•"}</Text>
              </View>
        ...
        <Pressable onPress={() => router.push("/ui/UI-15" as Href)} style={({ pressed }) => [styles.shareCard, pressed && styles.pressed]}>
          <View style={styles.shareIcon}>
            <Text style={styles.shareIconText}>礼</Text>
          </View>
          <View style={styles.shareCopy}>
            <Text style={styles.shareTitle}>创建一份邀请说明草稿</Text>
            <Text style={styles.shareText}>先检查隐私内容，不会直接发送给联系人</Text>
          </View>
          <Text style={styles.shareAction}>去了解</Text>
        </Pressable>
        ...
        <View style={styles.infoCard}>
          ...
          <View style={styles.courseList}>
            {BASELINE_COURSE_SECTIONS.map((section) => (
              <CourseSection key={section.title} title={section.title} items={section.items} />
            ))}
          </View>
        </View>
      ...
      <View style={styles.actionDock}>
        ...
        <Pressable disabled={submitState === "submitting"} onPress={saveIntent} style={({ pressed }) => [styles.buyButton, pressed && styles.pressed]}>
          <Text style={styles.buttonText}>{submitState === "submitting" ? "正在保存" : showIntentState ? "意向已保存" : "保存方案意向"}</Text>
        </Pressable>
        <Pressable onPress={() => router.push(`/ui/UI-16?productRef=${encodeURIComponent(product.productRef)}` as Href)} style={({ pressed }) => [styles.groupButton, pressed && styles.pressed]}>
          <Text style={styles.buttonText}>了解同行计划</Text>
        </Pressable>
      </View>
    </ScreenContainer>
  );
...
const styles = StyleSheet.create({
  ...
  scroll: { flex: 1, backgroundColor: "#FFF9F3" },
  ...
  catalogBoundary: { color: "#7B6848", fontSize: 12, lineHeight: 18, marginTop: 3 },
  intentBadge: {
