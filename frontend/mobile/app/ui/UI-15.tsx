...
export default function InvitationRewardsScreen() {
  ...
  const saveDraft = async (entry: string) => {
    ...
    await familyApi
      .recordDevFlowEvent(
        ...
        {
          ui_id: "UI-15",
          command: "SAVE_SYNTHETIC_INVITATION_DRAFT",
          selection: product.productRef,
        },
        createMobileRequestId("family-mobile-ui15"),
      )
      .catch((error) => {
        console.error("UI-15 remote action failed", error);
      });
  };
  ...
  return (
    <ScreenContainer edges={["left", "right", "bottom"]}>
      ...
      <ScrollView contentContainerStyle={[styles.content, { backgroundColor: colors.background }]} refreshControl={<FamilyRefreshControl />}>
        <View style={styles.header}>
          <View style={styles.topBar}>
            <Pressable onPress={() => router.back()} style={styles.backButton}>
              <IconSymbol name="chevron.left" size={26} color={colors.text} />
            </Pressable>
            <Text style={[styles.topTitle, { color: colors.text }]}>邀请说明</Text>
            <Text style={[styles.more, { color: colors.text }]}>•••</Text>
          </View>
          <View style={styles.hero}>
            <Text style={styles.title}>创建一份家庭邀请说明</Text>
            <Text style={styles.subtitle}>先保存私有草稿，再决定下一步</Text>
          </View>
        </View>

        <View style={[styles.progressCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <View style={styles.progressTopline}>
            <Text style={[styles.progressLabel, { color: colors.text }]}>邀请说明草稿</Text>
            <Text style={[styles.progressValue, { color: colors.trust }]}>{state.invitationDraft ? "已保存" : "未保存"}</Text>
          </View>
          <Text style={[styles.progressHint, { color: colors.muted }]}>仅保存在家庭空间，不会打开外部应用或发送消息。</Text>
        </View>

        <Text style={[styles.sectionTitle, { color: colors.text }]}>可了解的权益说明</Text>
        ...
        <Pressable onPress={() => void saveDraft("邀请说明")} style={({ pressed }) => [styles.primaryButton, pressed && styles.pressed]}>
          ...
          <Text style={styles.primaryButtonText}>保存邀请草稿</Text>
        </Pressable>

        <Text style={[styles.methodTitle, { color: colors.text }]}>选择草稿模板</Text>
        <View style={styles.methodRow}>
          <InviteMethod label="好友说明" icon="person.2.fill" color="#16866D" onPress={() => void saveDraft("好友说明草稿")} />
          <InviteMethod label="动态文案" icon="safari.fill" color="#F2A325" onPress={() => void saveDraft("动态文案草稿")} />
          <InviteMethod label="海报文案" icon="gift.fill" color="#5D78D6" onPress={() => void saveDraft("海报文案草稿")} />
        </View>

        <View style={[styles.brandBanner, { backgroundColor: "#EAF2FF" }]}>
          <View style={styles.brandCopy}>
            <Text style={styles.brandTitle}>一起成长，一起变好</Text>
            <Text style={[styles.brandText, { color: colors.muted }]}>私有草稿 · 权益待确认 · 不自动外发</Text>
          </View>
        ...
        <Pressable onPress={() => router.push(`/ui/UI-14?productRef=${encodeURIComponent(product.productRef)}` as Href)} style={({ pressed }) => [styles.secondaryButton, { borderColor: colors.border }, pressed && styles.pressed]}>
          <Text style={[styles.secondaryButtonText, { color: colors.trust }]}>返回方案详情</Text>
        </Pressable>
      </ScrollView>
    </ScreenContainer>
  );
...
const styles = StyleSheet.create({
  ...
  title: { color: "#0078D4", fontSize: 24, lineHeight: 34, fontWeight: "800" },
  subtitle: {
    color: "#63809E",
    fontSize: 14,
    lineHeight: 21,
  ...
  rewardTitle: { fontSize: 12, lineHeight: 18, fontWeight: "700" },
  rewardDetail: { fontSize: 12, lineHeight: 18 },
  primaryButton: {
    minHeight: 56,
    borderRadius: 18,
