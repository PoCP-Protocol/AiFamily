...
export default function MembershipCenterScreen() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((error) => {
        console.error("UI-18 remote projection failed", error);
      });
    return () => {
      active = false;
    };
        ...
        ListHeaderComponent={
          <View style={styles.header}>
            <View style={styles.topBar}>
              ...
              <Text style={[styles.topTitle, { color: colors.text }]}>我的</Text>
              ...
              <View style={styles.profileTop}>
                ...
                <View style={styles.profileCopy}>
                  <View style={styles.nameRow}>
                    ...
                    <Text style={styles.partnerBadge}>家庭成长伙伴</Text>
                  </View>
                  <Text style={styles.profileMotto}>一起成长，一起影响更多家庭</Text>
                </View>
              </View>
              <View style={styles.statsRow}>
                <Stat label="邀请草稿" value={state.invitationDraft ? "1" : "0"} />
                <Stat label="同行草稿" value={state.studyGroupDraft?.state === "PRIVATE_DRAFT" ? "1" : "0"} />
                <Stat label="积分投影" value={points === null ? "--" : String(points)} />
            ...
            <View style={[styles.levelCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              <View style={styles.levelCopy}>
                <Text style={[styles.levelLabel, { color: colors.muted }]}>会员状态</Text>
                <Text style={{ color: colors.text, fontSize: 17, lineHeight: 24, fontWeight: "900" }}>只读权益投影</Text>
                <Text style={[styles.levelHint, { color: colors.muted }]}>只显示已确认的会员与权益，不计算家庭等级。</Text>
              </View>
              <IconSymbol name="shield.fill" size={44} color={colors.trust} />
            </View>
          </View>
        }
        renderItem={({ item }) => (
          <Pressable onPress={() => router.push(`/ui/${item.target}` as Href)} style={({ pressed }) => [styles.menuRow, { backgroundColor: colors.surface, borderColor: colors.border }, pressed && styles.pressed]}>
                ...
                {annualPlan.benefits.map((benefit) => (
                  <View key={benefit.benefit_ref} style={styles.benefitRow}>
                    ...
                    <Text style={[styles.benefitText, { color: colors.muted }]}>
                      {benefit.title} · {benefit.units_per_grant} {benefit.allocation_type === "COUNT" ? "次" : "项"}
                    </Text>
                  </View>
                ))}
              </View>
...
const styles = StyleSheet.create({
  ...
  benefitTitle: { fontSize: 15, lineHeight: 22, fontWeight: "700" },
