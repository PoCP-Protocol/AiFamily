  ...
  return (
    <View style={[styles.shell, { backgroundColor: colors.background }]}>
      <aside style={styles.leftRail as never} aria-label="Family AI 主导航">
        ...
        >
          <Pressable accessibilityRole="button" accessibilityLabel="返回 Family AI 首页" onPress={() => navigate("/")} style={styles.brand}>
            ...
            <View>
              <Text style={[styles.brandName, { color: colors.text }]}>Family AI</Text>
              <Text style={[styles.brandTagline, { color: colors.muted }]}>一起成为更好的家</Text>
            </View>
          </Pressable>
            ...
            {FAMILY_PRIMARY_NAV.map((item) => {
              ...
              return (
                <Pressable
                  ...
                  style={({ pressed }) => [styles.navItem, active && { backgroundColor: `${colors.trust}14` }, pressed && styles.pressed]}
                >
                  <IconSymbol name={item.icon} size={23} color={active ? colors.trust : colors.text} />
                  <Text style={[styles.navLabel, { color: active ? colors.trust : colors.text }]}>{item.label}</Text>
                </Pressable>
              );
            })}
          </View>
          ...
          <View style={[styles.shortcutGroup, { borderTopColor: colors.border }]}>
            ...
            {FAMILY_SHORTCUTS.map((item) => (
              <Pressable key={item.route} accessibilityRole="link" onPress={() => navigate(item.route)} style={({ pressed }) => [styles.shortcut, pressed && styles.pressed]}>
                <Text style={[styles.shortcutText, { color: colors.muted }]}>{item.label}</Text>
      ...
      <main style={styles.main as never} aria-label="Family AI 主要内容">
        <View style={[styles.stageFrame, { backgroundColor: "#F3ECE5" }]}>
          <View
            style={[
              styles.stage,
              {
      ...
      {wideDesktop ? (
        <aside style={styles.rightRail as never} aria-label="家庭成长上下文">
          <ScrollView contentContainerStyle={styles.rightRailInner} showsVerticalScrollIndicator={false}>
            <View style={[styles.contextCard, { borderColor: colors.border, backgroundColor: colors.surface }]}>
              <View style={styles.contextHeader}>
                <View style={[styles.familyAvatar, { backgroundColor: `${colors.trust}18` }]}>
                  <IconSymbol name="person.2.fill" size={22} color={colors.trust} />
                </View>
                <View>
                  <Text style={[styles.contextEyebrow, { color: colors.muted }]}>当前家庭</Text>
                  <Text style={[styles.contextTitle, { color: colors.text }]}>我们的成长空间</Text>
                </View>
              </View>
            ...
            <View style={[styles.contextCard, { borderColor: colors.border, backgroundColor: colors.surface }]}>
              ...
              <Pressable accessibilityRole="link" onPress={() => navigate("/ui/UI-09")} style={styles.contextLink}>
                <Text style={[styles.contextLinkText, { color: colors.tint }]}>打开今日行动</Text>
                <IconSymbol name="chevron.right" size={16} color={colors.tint} />
              </Pressable>
            </View>

            <View style={[styles.trustNote, { backgroundColor: `${colors.trust}0B` }]}>
              <IconSymbol name="shield.fill" size={18} color={colors.trust} />
...
const styles = StyleSheet.create({
  ...
  brandName: { fontSize: 18, lineHeight: 24, fontWeight: "800" },
  ...
  navLabel: { fontSize: 16, lineHeight: 22, fontWeight: "700" },
  shortcutGroup: {
    marginTop: 20,
    paddingTop: 18,
  ...
  stageFrame: {
    width: IPHONE_16_PRO_MAX_WIDTH + 28,
    minHeight: IPHONE_16_PRO_MAX_HEIGHT + 28,
    borderRadius: 38,
    padding: 14,
    shadowColor: "#2B5C90",
    shadowOpacity: 0.16,
  ...
  contextTitle: { fontSize: 15, lineHeight: 22, fontWeight: "700" },
  contextBody: { fontSize: 13, lineHeight: 20, fontWeight: "400" },
  contextLink: {
    minHeight: 34,
    flexDirection: "row",
    alignItems: "center",
  ...
  contextLinkText: { fontSize: 13, lineHeight: 20, fontWeight: "700" },
  trustNote: {
    borderRadius: 15,
    padding: 14,
    flexDirection: "row",
  ...
  trustText: { flex: 1, fontSize: 12, lineHeight: 18, fontWeight: "400" },
