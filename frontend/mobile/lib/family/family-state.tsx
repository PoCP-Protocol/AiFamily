...
import {
  ...
  type FamilyMobileState,
  type FamilyFlowSource,
  type FamilyRemoteSyncState,
} from "./family-state-core";
...
interface FamilyMobileContextValue extends FamilyMobileState {
  ...
  saveAssessmentNeed(text: string): void;
  restartAssessment(): void;
  setAssessmentStep(step: AssessmentFlowStep): void;
...
const FamilyMobileContext = createContext<FamilyMobileContextValue | null>(
  null,
);
...
export function FamilyMobileProvider({ children }: PropsWithChildren) {
  ...
  useEffect(() => {
    ...
    const finishHydration = (
      payload: Record<string, never> | FamilyMobileState,
    ) => {
  ...
  const value = useMemo<FamilyMobileContextValue>(
    () => ({
      ...
      selectGrowthFocus: (focus) =>
        dispatch({ type: "select_growth_focus", focus }),
      saveAssessmentNeed: (text) =>
        dispatch({ type: "save_assessment_need", text }),
      restartAssessment: () => dispatch({ type: "restart_assessment" }),
      setAssessmentStep: (step) =>
        dispatch({ type: "set_assessment_step", step }),
      answerAssessment: (questionId, answer) =>
        dispatch({ type: "answer_assessment", questionId, answer }),
      setAssessmentSyncState: (nextState) =>
        dispatch({ type: "set_assessment_sync", state: nextState }),
      setActiveOnboardingId: (onboardingId) =>
        dispatch({ type: "set_active_onboarding", onboardingId }),
      recordChildChoice: (promptId, choice) =>
        dispatch({ type: "record_child_choice", promptId, choice }),
      savePrivateGrowthStory: (draft) =>
        dispatch({ type: "save_private_growth_story", draft }),
      saveCommerceIntentDraft: (productRef, productVersion, productTitle) =>
        dispatch({
      ...
      syncCommerceIntentReceipt: (intentId, entitlementId) =>
        dispatch({
          type: "sync_commerce_intent_receipt",
          intentId,
          entitlementId,
        }),
      saveInvitationDraft: (productRef, productTitle) =>
        dispatch({ type: "save_invitation_draft", productRef, productTitle }),
      saveStudyGroupDraft: (productRef, productTitle, familyCount) =>
        dispatch({
      ...
      cancelStudyGroupDraft: () =>
        dispatch({ type: "cancel_study_group_draft" }),
      saveConsultationNeedDraft: (
        offeringRef,
        offeringVersion,
        offeringTitle,
      ...
      syncConsultationNeedReceipt: (bookingRequestId, serviceRecordId) =>
        dispatch({
          type: "sync_consultation_need_receipt",
          bookingRequestId,
          serviceRecordId,
        }),
      saveActivityInterestDraft: (activityRef, activityTitle) =>
        dispatch({
      ...
      toggleCommunityBookmark: (exchangeRef) =>
        dispatch({ type: "toggle_community_bookmark", exchangeRef }),
      toggleCommunityFollow: (exchangeRef) =>
        dispatch({ type: "toggle_community_follow", exchangeRef }),
  ...
  return (
    <FamilyMobileContext.Provider value={value}>
      {children}
    </FamilyMobileContext.Provider>
  );
}
...
export function useFamilyMobile() {
  ...
  if (!value)
    throw new Error("useFamilyMobile must be used within FamilyMobileProvider");
  return value;
}
