...
export class FamilyApiClient {
  ...
  getLatestAssessmentSupportCard<T>(token: string, familyId: string) {
    return this.request<T>(`/families/${familyId}/assessments/support-card/latest`, { token });
  }

  submitAssessmentSupportCardFeedback<T>(
    token: string,
    familyId: string,
    body: {
    ...
    return this.request<T>(`/families/${familyId}/assessments/support-card/feedback`, {
      ...
      headers: {
        "idempotency-key": idempotencyKey,
        "x-correlation-id": createMobileRequestId("ui03-support-feedback"),
        "x-source": "family-ai-mobile",
      },
    });
  }

  startAssessmentSupportCardSmallStep<T>(
    token: string,
    familyId: string,
    body: { assessment_session_id: string; action_ref: "TRY_TONIGHT" },
    idempotencyKey: string,
  ) {
    return this.request<T>(`/families/${familyId}/assessments/support-card/small-step`, {
      ...
      headers: {
        "idempotency-key": idempotencyKey,
        "x-correlation-id": createMobileRequestId("ui03-support-small-step"),
        "x-source": "family-ai-mobile",
      },
    });
  }

  recordAssessmentSupportCardCheckin<T>(
    token: string,
    familyId: string,
    body: {
      assessment_session_id: string;
    ...
    return this.request<T>(`/families/${familyId}/assessments/support-card/checkins`, {
      ...
      headers: {
        "idempotency-key": idempotencyKey,
        "x-correlation-id": createMobileRequestId("ui03-support-checkin"),
        "x-source": "family-ai-mobile",
      },
    });
  }
