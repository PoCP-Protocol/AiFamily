...
import {
  buildFamilyRhythmEvents,
  buildPrivateGrowthStory,
  CHILD_PRACTICE_PROMPTS,
} from "../lib/family/child-growth";
import {
  commerceProductsForDisplay,
  EXISTING_COMMERCE_PRESENTATION,
} from "../lib/family/commerce-entitlements";
import {
  communityEntriesForDisplay,
  detectCommunityPrivacyRisks,
} from "../lib/family/community-content";
import {
  assessmentCompletion,
  GROWTH_FOCUSES,
} from "../lib/family/core-growth";
import {
  FamilyApiClient,
  FamilyApiError,
...
import {
  ...
  getJourneyWeeklyAction,
} from "../lib/family/journey-plan-content";
import {
  growthActivitiesForDisplay,
  serviceOfferingsForDisplay,
} from "../lib/family/service-support";
import { UI_ACTION_POLICIES } from "../lib/family/ui-action-policies";
import {
  FAMILY_SCREENS,
  getFamilyScreen,
  getScreensForTab,
} from "../lib/family/ui-registry";
...
describe("Family AI UI registry", () => {
  ...
  it("places every screen in one of the five mobile tabs", () => {
    const total = ["today", "growth", "discover", "services", "mine"]
      .map(
        (tab) =>
          getScreensForTab(tab as Parameters<typeof getScreensForTab>[0])
            .length,
      )
  ...
  it("keeps a named visual baseline and feature set for every baseline screen and the assessment result route", () => {
    expect(
      FAMILY_SCREENS.every((screen) => screen.baseline.trim().length > 0),
    ).toBe(true);
    expect(
      FAMILY_SCREENS.every((screen) => screen.featurePoints.length >= 4),
    ).toBe(true);
    expect(
      FAMILY_SCREENS.slice(12, 18).map((screen) => screen.baseline),
    ).toEqual([
    ...
    expect(
      FAMILY_SCREENS.slice(18, 24).map((screen) => screen.baseline),
    ).toEqual([
      "teacher-zone",
      "teacher-detail",
    ...
    expect(
      FAMILY_SCREENS.slice(24, 28).map((screen) => screen.baseline),
    ).toEqual([
      "parent-community",
      "publish-dynamic",
      "dynamic-detail",
...
describe("UI-01 to UI-34 global navigation and state readback", () => {
  ...
  it("carries growth, assessment, service, commerce and community state into later readback surfaces", () => {
    const focused = familyMobileReducer(initialFamilyMobileState, {
      type: "select_growth_focus",
      focus: "PARENT_CHILD_COMMUNICATION",
    });
    const assessed = familyMobileReducer(focused, {
      type: "answer_assessment",
      questionId: GROWTH_FOCUSES[0].questions[0].id,
      answer: "sometimes",
    });
    const camp = familyMobileReducer(assessed, {
      type: "activate_camp_day",
      day: 1,
    });
    const completed = familyMobileReducer(camp, {
      type: "complete_action",
      reflection: "我先停下来听完。",
    });
    const service = familyMobileReducer(completed, {
    ...
    const commerce = familyMobileReducer(service, {
      type: "save_commerce_intent_draft",
      productRef: "TEST_ANNUAL_COMPANION",
      productVersion: 1,
      productTitle: "年度陪伴",
    });
    const community = familyMobileReducer(commerce, {
    ...
    expect(community.assessmentAnswers).toHaveProperty(
      GROWTH_FOCUSES[0].questions[0].id,
      "sometimes",
    );
    expect(community.lastReceipt).toMatchObject({
      reflectionKind: "perspective",
      externalEffect: false,
    });
    expect(community.consultationNeedDraft).toMatchObject({
      state: "LOCAL_DRAFT",
      externalEffect: false,
    });
    expect(community.commerceIntentDraft).toMatchObject({
      state: "LOCAL_DRAFT",
      externalEffect: false,
    });
    expect(community.communityPostDraft).toMatchObject({
      visibility: "FAMILY_PRIVATE",
      perspectiveKind: "PARENT_PERSPECTIVE_NOT_FACT",
      externalEffect: false,
    });
    expect(community.communityPostDraft?.aiTagDraft).toMatchObject({
      tags: ["亲子沟通", "日常行动"],
      factBoundary: "TAGS_ARE_EDITABLE_PERSPECTIVE_NOT_FACT",
    });
  });
...
describe("21-day parent growth camp", () => {
  it("contains 21 ordered daily practices across three seven-day stages", () => {
    ...
    expect(CAMP_21_DAYS.map((day) => day.day)).toEqual(
      Array.from({ length: 21 }, (_, index) => index + 1),
    );
    expect(
      CAMP_21_DAYS.filter((day) => day.stage === "观察与连接"),
    ).toHaveLength(7);
    expect(
      CAMP_21_DAYS.filter((day) => day.stage === "沟通与习惯"),
    ).toHaveLength(7);
    expect(
      CAMP_21_DAYS.filter((day) => day.stage === "反思与延续"),
    ).toHaveLength(7);
  });
...
describe("Today action and camp check-in state", () => {
  ...
  it("records an action receipt without turning reflection into fact or external effect", () => {
    const activated = familyMobileReducer(initialFamilyMobileState, {
      type: "activate_camp_day",
      day: 1,
    });
    const started = familyMobileReducer(activated, { type: "start_action" });
    const completed = familyMobileReducer(started, {
      type: "complete_action",
      reflection: "我先听完了。",
    });
  ...
  it("is idempotent for the completed-day ledger", () => {
    ...
    const first = familyMobileReducer(activated, {
      type: "complete_action",
      reflection: "第一次",
    });
    const replay = familyMobileReducer(first, {
      type: "complete_action",
      reflection: "重放",
    });
...
describe("six-loop controlled mobile actions", () => {
  it("covers growth, plan, assessment, service, commerce, and community across registry and policies", () => {
    const loops = new Set([
      ...
      ...Object.values(UI_ACTION_POLICIES)
        .filter(Boolean)
        .map((policy) => policy!.loop),
    ]);

    expect(loops).toEqual(
      new Set(["成长", "计划", "评估", "服务", "商业", "社区"]),
    );
  });
  ...
  it("records a service intent as an idempotent no-external-effect receipt", () => {
    ...
    expect(replay.uiActionReceipts[0]).toMatchObject({
      screenId: "UI-21",
      kind: "service_intent",
      externalEffect: false,
    });
...
describe("family assessment and 90-day journey", () => {
  it("keeps five bounded family focus areas with three scenario questions each", () => {
    ...
    expect(GROWTH_FOCUSES.every((focus) => focus.questions.length === 3)).toBe(
      true,
    );
  });
  ...
  it("stores parent answers as local perspective inputs and tracks completion", () => {
    ...
    expect(
      assessmentCompletion(
        answered.selectedGrowthFocus,
        answered.assessmentAnswers,
      ),
    ).toBe(1);
    expect(answered.assessmentSyncState).toBe("local");
    expect(Object.values(answered.assessmentAnswers)).toEqual([
      "sometimes",
      "sometimes",
      "sometimes",
    ]);
  ...
  it("persists the conversation entry and resumable step as a private draft", () => {
    const withNeed = familyMobileReducer(initialFamilyMobileState, {
      type: "save_assessment_need",
      text: "晚上写作业总是容易吵起来",
    });
    const resumed = familyMobileReducer(withNeed, {
      type: "set_assessment_step",
      step: "questions",
    });

    expect(resumed.assessmentNeedText).toBe("晚上写作业总是容易吵起来");
    expect(resumed.assessmentFlowStep).toBe("questions");
    expect(resumed.assessmentSyncState).toBe("local");
    expect(resumed).not.toHaveProperty("familyNeed");
  });

  it("clears only the local assessment draft when restarting", () => {
    const started = familyMobileReducer(
      familyMobileReducer(
        familyMobileReducer(initialFamilyMobileState, {
          type: "save_assessment_need",
          text: "今晚写作业容易吵起来",
        }),
        { type: "select_growth_focus", focus: "LEARNING_HABITS" },
      ),
      { type: "answer_assessment", questionId: "learning-1", answer: "often" },
    );
    const restarted = familyMobileReducer(started, {
      type: "restart_assessment",
    });

    expect(restarted.assessmentNeedText).toBe("");
    expect(restarted.selectedGrowthFocus).toBeNull();
    expect(restarted.assessmentAnswers).toEqual({});
    expect(restarted.assessmentFlowStep).toBe("story");
    expect(restarted.assessmentSyncState).toBe("local");
    expect(restarted.todayAction).toEqual(started.todayAction);
  });

  it("uses four ordered phases and focus-specific weekly actions", () => {
    expect(MOBILE_JOURNEY_PHASES.map((phase) => phase.id)).toEqual([
      "SEE",
      "PARENT_FIRST",
      "CO_CREATE",
      "STABILIZE",
...
describe("Family API mobile contract", () => {
  it("sends idempotency metadata when issuing a dev account session", async () => {
    const fetcher = vi.fn(
      async () =>
        new Response(
          JSON.stringify({ token: "dev-token", expires_at: "2099-01-01T00:00:00Z", account_id: "account-1" }),
          { status: 200, headers: { "Content-Type": "application/json" } },
        ),
    ) as unknown as typeof fetch;
    const client = new FamilyApiClient("https://family.example", fetcher);

    await client.issueDevAccountSession("account-1:family-1");

    const [url, request] = vi.mocked(fetcher).mock.calls[0];
    expect(url).toBe("https://family.example/auth/account-session");
    expect(request?.method).toBe("POST");
    expect(request?.headers).toMatchObject({
      "idempotency-key": expect.stringContaining("family-mobile-dev-session-"),
      "x-correlation-id": expect.stringContaining("family-mobile-dev-session-"),
  ...
  it("uses account Bearer, omits cookies, and calls family contexts", async () => {
    const fetcher = vi.fn(
      async () =>
        new Response(
          JSON.stringify({ account_id: "account-1", contexts: [] }),
          {
            status: 200,
  ...
  it("sends an idempotency key for private service drafts without external-effect fields", async () => {
    const fetcher = vi.fn(
      async () =>
        new Response(
          JSON.stringify({ state: "CREATED", external_effect: false }),
          {
            status: 200,
            headers: { "Content-Type": "application/json" },
          },
    ...
    await client.createPrivateCheckinDraft(
      "fam_token",
      "family-1",
      "onboarding-1",
      "WEEKLY_ACTION_SEE",
    ...
    expect(url).toContain(
      "/families/family-1/growth/onboardings/onboarding-1/service-journey/checkin-drafts",
    );
    expect(request?.method).toBe("POST");
    expect(request?.headers).toMatchObject({
  ...
  it("reads post-UI-10 surfaces from the same family-scoped platform projection", async () => {
    ...
    const [url, request] = vi.mocked(fetcher).mock.calls[0];
    expect(url).toBe(
      "https://family.example/families/family-1/dev/platform-surfaces",
    );
    expect(request?.credentials).toBe("omit");
    expect(request?.headers).toMatchObject({
      Authorization: "Bearer fam_token",
  ...
  it("records only an idempotent UI-26 no-op receipt without publishing content or contact fields", async () => {
    const fetcher = vi.fn(
      async () =>
        new Response(
          JSON.stringify({ external_effect: false, status: "RECORDED" }),
          {
            status: 201,
            headers: { "Content-Type": "application/json" },
          },
    ...
    await client.recordDevFlowEvent(
      "fam_token",
      "family-1",
      {
        ui_id: "UI-26",
    ...
    expect(url).toBe(
      "https://family.example/families/family-1/dev/flow-events",
    );
    expect(request?.headers).toMatchObject({
      "idempotency-key": "community-idempotency-1",
      Authorization: "Bearer fam_token",
    });
    expect(body).toEqual({
      ui_id: "UI-26",
    ...
    expect(JSON.stringify(body)).not.toMatch(
      /title|body|media|publish|comment|recipient|notification|phone|address|child_name/i,
    );
  });
  ...
  it("reads commerce and membership projections from the existing family-scoped API", async () => {
    const fetcher = vi.fn(
      async () =>
        new Response(
          JSON.stringify({ products: [], plans: [], subscriptions: [] }),
          {
            status: 200,
            headers: { "Content-Type": "application/json" },
          },
  ...
  it("submits only a versioned UI-14 intent with idempotency and no payment fields", async () => {
    const fetcher = vi.fn(
      async () =>
        new Response(
          JSON.stringify({
            intent: { external_effect: false },
            entitlement: { external_effect: false },
          }),
          {
            status: 201,
            headers: { "Content-Type": "application/json" },
    ...
    await client.submitCommerceIntent(
      "fam_token",
      "family-1",
      {
        page_id: "UI-14",
    ...
    const body = JSON.parse(request?.body as string);
    expect(url).toContain(
      "/families/family-1/orchestration/test-loop/commerce/order-intents",
    );
    expect(request?.method).toBe("POST");
    expect(request?.headers).toMatchObject({
      "idempotency-key": "intent-idempotency-1",
      Authorization: "Bearer fam_token",
    });
    ...
    expect(JSON.stringify(body)).not.toMatch(
      /payment|price|currency|contact|recipient|external_effect/i,
    );
  });
  ...
  it("reads service supply, slots, and family-private customer projection from the existing Family API", async () => {
    ...
    await client.getServiceSlots(
      "fam_token",
      "family-1",
      "TEST_PARENT_CHILD_DIALOGUE",
      1,
    );
  ...
  it("submits only an idempotent UI-21 service request with no payment, notification, provider, or raw child fields", async () => {
    const fetcher = vi.fn(
      async () =>
        new Response(
          JSON.stringify({
            booking: { external_effect: false },
            service_record: { external_effect: false },
          }),
          {
            status: 201,
            headers: { "Content-Type": "application/json" },
    ...
    await client.submitServiceBooking(
      "fam_token",
      "family-1",
      {
        page_id: "UI-21",
        ...
        attributes: {
          entry: "family_ai_mobile_consultation_need",
          channel_preference: "VIDEO",
        },
      },
    ...
    const body = JSON.parse(request?.body as string);
    expect(url).toBe(
      "https://family.example/families/family-1/orchestration/test-loop/services/booking-requests",
    );
    expect(request?.method).toBe("POST");
    expect(request?.headers).toMatchObject({
      "idempotency-key": "service-idempotency-1",
      Authorization: "Bearer fam_token",
    });
    expect(body).toMatchObject({
      page_id: "UI-21",
    ...
    expect(JSON.stringify(body)).not.toMatch(
      /payment|price|currency|recipient|notification|provider_ref|child_name|phone|address|diagnosis/i,
    );
  });
  ...
  it("surfaces required-mode 401 errors instead of silently falling back to another family", async () => {
    const fetcher = vi.fn(
      async () =>
        new Response(JSON.stringify({ message: "bearer_token_required" }), {
          status: 401,
          headers: { "Content-Type": "application/json" },
        }),
    ) as unknown as typeof fetch;
...
describe("child autonomy and private family storytelling", () => {
  it("selects UI-10 through UI-12 from the existing Family API projections without redefining their boundaries", () => {
    ...
    const journey = {
      ...
      headline: "我们已经走过的几步",
      entries: [
        { event_id: "event-1", label: "记录行动", detail: "保留过程。" },
      ],
    ...
    expect(selectChildActionPrompt(core)?.fact_boundary).toBe(
      "ACTION_RECORDED_NOT_CHILD_OUTCOME",
    );
    expect(selectPersonalGrowthJourney(platform)?.fact_boundary).toBe(
      "PROCESS_EVENTS_NOT_OUTCOME_OR_RANKING",
    );
    expect(selectPrivateGrowthStory(platform)?.fact_boundary).toBe(
      "PROCESS_EVENTS_NOT_OUTCOME_OR_SHARE",
    );
  });
  ...
  it("offers try, change, and pause on every child prompt without score or reward fields", () => {
    ...
    for (const prompt of CHILD_PRACTICE_PROMPTS) {
      expect(prompt.choices.map((choice) => choice.id)).toEqual([
        "TRY_THIS",
        "CHOOSE_ANOTHER",
        "PAUSE_TODAY",
      ]);
      expect(JSON.stringify(prompt)).not.toMatch(
        /score|rank|reward|badge|streak/i,
      );
    }
  });
  ...
  it("records pause as a private child perspective rather than fact, failure, or external effect", () => {
    ...
    expect(JSON.stringify(paused.childChoiceDraft)).not.toMatch(
      /failure|ability|diagnosis|score|rank/i,
    );
  });
  ...
  it("builds only same-family process events without ranking, points, percentile, or child outcomes", () => {
    ...
    expect(
      events.every(
        (event) => event.evidenceBoundary === "process_event_not_outcome",
      ),
    ).toBe(true);
    expect(events.flatMap((event) => Object.keys(event))).not.toEqual(
    ...
    ).not.toMatch(/排名|总分|百分位|积分|勋章|奖励|成长结果/);
  });
  ...
  it("creates a maximum four-event private story draft with no media, share, publish, or notification effect", () => {
    ...
    expect(JSON.stringify(story)).not.toMatch(
      /share|publish|recipient|notification|download|qrcode|photo|school|child_name/i,
    );
  });
});
...
describe("commerce and membership mobile presentation", () => {
  it("merges admitted Family API products into the existing Web baseline without dropping baseline screens", () => {
    ...
    expect(display.map((item) => item.title)).toEqual(
      expect.arrayContaining([
        "家庭成长测评卡",
        "亲子阅读工具包",
        "家庭专注力提升训练营",
  ...
  it("keeps commerce, invitation, and study-group drafts family private with no external effect", () => {
    ...
    expect(group.commerceIntentDraft).toMatchObject({
      state: "LOCAL_DRAFT",
      visibility: "FAMILY_PRIVATE",
      externalEffect: false,
    });
    expect(group.invitationDraft).toMatchObject({
      state: "PRIVATE_DRAFT",
      visibility: "FAMILY_PRIVATE",
      externalEffect: false,
    });
    expect(group.studyGroupDraft).toMatchObject({
      familyCount: 3,
      state: "PRIVATE_DRAFT",
      visibility: "FAMILY_PRIVATE",
      externalEffect: false,
    ...
    expect(
      JSON.stringify({
        commerce: group.commerceIntentDraft,
        invitation: group.invitationDraft,
        group: group.studyGroupDraft,
      }),
    ).not.toMatch(
      /payment|charged|contact|recipient|notification|share_url|qrcode/i,
    );
  });
  ...
  it("syncs only the existing intent and entitlement receipt identities", () => {
    ...
    expect(synced.commerceIntentDraft).toMatchObject({
...
describe("expert service and family activity mobile presentation", () => {
  it("maps admitted teacher supply into the existing teacher-zone baseline without inventing provider facts", () => {
    ...
    expect(JSON.stringify(display[0])).not.toMatch(
      /score|rating|rank|recommended_for_family|diagnosis/i,
    );
  });
  ...
  it("selects the UI-22 activity catalog from the same platform projection and keeps browsing separate from registration", () => {
    const catalog = {
      ...
      activities: [
        {
          activity_ref: "ACTIVITY_PARENT_CHILD_DIALOGUE",
          title: "亲子沟通小练习",
          summary: "交换彼此的想法。",
          age_hint: "学龄儿童家庭",
    ...
    expect(selected?.fact_boundary).toBe(
      "ACTIVITY_BROWSING_NOT_REGISTRATION_ATTENDANCE_OR_OUTCOME",
    );
    expect(display[0]).toMatchObject({
  ...
  it("keeps consultation needs and activity interests family private without external messaging or outcome claims", () => {
    ...
    const activity = familyMobileReducer(consultation, {
      type: "save_activity_interest_draft",
      activityRef: "ACTIVITY_PARENT_CHILD_DIALOGUE",
      activityTitle: "亲子沟通小练习",
    });

    expect(activity.consultationNeedDraft).toMatchObject({
      state: "LOCAL_DRAFT",
      visibility: "FAMILY_PRIVATE",
      consentAcknowledged: true,
      externalEffect: false,
    });
    expect(activity.activityInterestDraft).toMatchObject({
      state: "PRIVATE_DRAFT",
      visibility: "FAMILY_PRIVATE",
      externalEffect: false,
    });
    expect(
      JSON.stringify({
        consultation: activity.consultationNeedDraft,
        activity: activity.activityInterestDraft,
      }),
    ).not.toMatch(
      /recipient|notification|calendar_event|payment|confirmed_service|diagnosis|outcome|score|rank/i,
    );
  });
  ...
  it("syncs only booking and service-record receipt identities after the server accepts a request", () => {
    ...
    expect(synced.consultationNeedDraft).toMatchObject({
      state: "SYNCED_RECEIPT",
      bookingRequestId: "booking-1",
      serviceRecordId: "record-1",
      externalEffect: false,
...
describe("community content and private family notes", () => {
  it("selects the UI-25 moderated experience feed from the same platform projection", () => {
    const feed = {
      ...
      fact_boundary:
        "READING_EXPERIENCE_SUMMARIES_NOT_PUBLICATION_INTERACTION_OR_OUTCOME",
    } as const;
    const platform = {
      projection_version: "DEV_PLATFORM_SURFACES_V1",
      family_id: "family-1",
    ...
    expect(selectLearningExchangeFeed(platform)?.fact_boundary).toBe(
      "READING_EXPERIENCE_SUMMARIES_NOT_PUBLICATION_INTERACTION_OR_OUTCOME",
    );
    expect(
      selectLearningExchangeEntry(platform, "EXCHANGE_DIALOGUE_PAUSE")?.title,
    ).toBe("给一次对话留一点停顿");
    expect(communityEntriesForDisplay(feed.entries)[0]).toMatchObject({
      exchangeRef: "EXCHANGE_DIALOGUE_PAUSE",
      source: "FAMILY_API",
    });
  });
  ...
  it("stores a family note as parent perspective and never as a public post or fact", () => {
    ...
    expect(state.communityPostDraft).toMatchObject({
      state: "PRIVATE_DRAFT",
      visibility: "FAMILY_PRIVATE",
      perspectiveKind: "PARENT_PERSPECTIVE_NOT_FACT",
    ...
    expect(JSON.stringify(state.communityPostDraft)).not.toMatch(
      /published|public_url|media|recipient|notification|like_count|comment_count/i,
    );
  });

  it("keeps bookmark, follow, and response as one family-private interaction draft", () => {
    const bookmarked = familyMobileReducer(initialFamilyMobileState, {
      type: "toggle_community_bookmark",
      exchangeRef: "EXCHANGE_DIALOGUE_PAUSE",
    });
    const followed = familyMobileReducer(bookmarked, {
      type: "toggle_community_follow",
      exchangeRef: "EXCHANGE_DIALOGUE_PAUSE",
    });
    const responded = familyMobileReducer(followed, {
      type: "save_community_response_draft",
      exchangeRef: "EXCHANGE_DIALOGUE_PAUSE",
      responseText: "我想先试试停顿。",
    });
    ...
    expect(JSON.stringify(draft)).not.toMatch(
      /notification|recipient|like_count|comment_count|follower_count/i,
    );
  });

  it("flags direct family identifiers before a private note can be saved", () => {
    expect(
      detectCommunityPrivacyRisks("孩子在阳光小学三年级，电话 13812345678"),
    ).toEqual(
      expect.arrayContaining(["请移除手机号码", "请避免填写学校、班级或住址"]),
    );
