...
describe("UI-02 family conversation assessment contract", () => {
  ...
  it("keeps the commercial consent, evidence, and non-diagnosis boundary explicit", () => {
    ...
    expect(resultSource).toContain("下一步 · 进入 21 天家庭计划");
    ...
    expect(resultSource).toContain("家庭理解卡");
    expect(resultSource).toContain("assessment-observation-toggle");
    expect(resultSource).toContain("assessment-observation-layer");
    expect(resultSource).toContain("依据");
    expect(resultSource).toContain("可能的方向");
    expect(resultSource).toContain("还未知");
    ...
    expect(resultSource).not.toContain("assessment-start-small-step");
    expect(resultSource).toContain("进入 21 天家庭计划");
    expect(resultSource).toContain("不会自动创建计划或行动");
    ...
    expect(explanationSource).not.toContain("assessment-start-small-step");
    expect(explanationSource).toContain("进入 21 天家庭计划");
