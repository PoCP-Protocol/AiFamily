...
async def get_product_concept_chain(
    ...
    *,
    ...
    opportunity = (
        await repo.load_opportunity(problem.opportunity_id, tenant)
        if problem.opportunity_id
        else None
    )
    insight = (
        await repo.load_customer_insight(opportunity.insight_id, tenant) if opportunity else None
    )
    signal = (
        await repo.load_market_signal(insight.signal_id, tenant)
        if insight and insight.signal_id
        else None
    )
    return {
