...
async def test_ai_generated_hypothesis_starts_as_draft(fake_repo, human_context, ai_context):
    problem = await _make_problem(
        fake_repo, human_context
    )  # parent chain authored by a human; only the hypothesis itself is AI-generated
    hypothesis = await commands.create_growth_hypothesis(
        ...
        statement="h",
        model_ref="m",
        prompt_use_case_version="growth.hypothesis.generate@1",
        confidence=0.5,
    )
...
@pytest.mark.asyncio
async def test_ai_actor_creation_without_full_provenance_is_rejected(
    fake_repo, human_context, ai_context
):
    problem = await _make_problem(fake_repo, human_context)
    with pytest.raises(ProductIntelligenceValidationError):
        await commands.create_growth_hypothesis(
...
async def test_ai_actor_cannot_validate_hypothesis(fake_repo, human_context, ai_context):
    ...
    with pytest.raises(ProductIntelligenceForbiddenError):
        await commands.validate_growth_hypothesis(
            fake_repo, ai_context, hypothesis_id=hypothesis.id, reason="looks fine"
        )
...
async def test_human_actor_can_validate_hypothesis(fake_repo, human_context):
    ...
    validated = await commands.validate_growth_hypothesis(
        fake_repo, human_context, hypothesis_id=hypothesis.id, reason="matches evidence"
    )
...
async def test_retired_hypothesis_cannot_be_revalidated(fake_repo, human_context):
    ...
    with pytest.raises(ProductIntelligenceValidationError):
        await commands.validate_growth_hypothesis(
            fake_repo, human_context, hypothesis_id=retired.id, reason="x"
        )
...
async def test_validation_without_reason_is_rejected(fake_repo, human_context):
    ...
    with pytest.raises(ProductIntelligenceValidationError):
        await commands.validate_growth_hypothesis(
            fake_repo, human_context, hypothesis_id=hypothesis.id, reason=""
        )
