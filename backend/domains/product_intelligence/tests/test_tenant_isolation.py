...
@pytest.mark.asyncio
async def test_cross_tenant_insight_creation_is_rejected(
    fake_repo, human_context, other_tenant_human_context
):
    ...
    with pytest.raises(ProductIntelligenceNotFoundError):
        await commands.create_customer_insight(
            fake_repo,
            other_tenant_human_context,
            signal_id=signal.id,
            statement="tenant-b trying to chain off tenant-a's signal",
        )


@pytest.mark.asyncio
async def test_cross_tenant_load_market_signal_is_rejected(
    fake_repo, human_context, other_tenant_human_context
):
...
@pytest.mark.asyncio
async def test_cross_tenant_hypothesis_validation_is_rejected(
    fake_repo, human_context, other_tenant_human_context
):
    signal = await _make_signal(fake_repo, human_context)
    insight = await commands.create_customer_insight(
        fake_repo, human_context, signal_id=signal.id, statement="i"
    )
    opportunity = await commands.create_opportunity(
        fake_repo, human_context, insight_id=insight.id, statement="o"
    )
    problem = await commands.create_growth_problem(
        fake_repo, human_context, opportunity_id=opportunity.id, symptom="p"
    )
    ...
    with pytest.raises(ProductIntelligenceNotFoundError):
        await commands.validate_growth_hypothesis(
            ...
            reason="tenant-b should not reach this",
        )


@pytest.mark.asyncio
async def test_cross_tenant_chain_query_is_rejected(
    fake_repo, human_context, other_tenant_human_context
):
    signal = await _make_signal(fake_repo, human_context)
    insight = await commands.create_customer_insight(
    ...
    concept = await commands.create_product_concept(
        fake_repo, human_context, strategy_id=strategy.id, title="c"
    )

    with pytest.raises(ProductIntelligenceNotFoundError):
        await queries.get_product_concept_chain(
            fake_repo, other_tenant_human_context, product_concept_id=concept.id
        )
