...
async def test_human_with_no_permissions_is_forbidden(fake_repo):
    ...
    hypothesis = await commands.create_growth_hypothesis(
        fake_repo, context, problem_id=problem.id, statement="h"
    )

    with pytest.raises(ProductIntelligenceForbiddenError):
        await commands.validate_growth_hypothesis(
            fake_repo, context, hypothesis_id=hypothesis.id, reason="looks fine"
        )
...
async def test_human_with_unrelated_permissions_is_forbidden(fake_repo):
    context = ActorContext(
        ...
        permissions=frozenset(
            {"product_intelligence.market_signal.create", "some.other.permission"}
        ),
    )
    problem = await _make_problem(fake_repo, context)
    hypothesis = await commands.create_growth_hypothesis(
...
async def test_ai_actor_with_review_permission_is_still_forbidden(fake_repo):
    ...
    hypothesis = await commands.create_growth_hypothesis(
        fake_repo, creator_context, problem_id=problem.id, statement="h"
    )

    ai_context = ActorContext(
        actor_id="ai-use-case:market.insight.generate",
        actor_type="AI",
        tenant_scope="tenant-a",
        permissions=frozenset({REVIEW_PERMISSION}),
    )
...
async def test_system_actor_with_review_permission_is_still_forbidden(fake_repo):
    ...
    with pytest.raises(ProductIntelligenceForbiddenError):
        await commands.validate_growth_hypothesis(
            fake_repo, system_context, hypothesis_id=hypothesis.id, reason="looks fine"
        )
