...
async def _build_chain(repo, context):
    signal = await commands.create_market_signal(
        repo, context, raw_text="家长普遍反映每天辅导作业太累"
    )
    insight = await commands.create_customer_insight(
        repo, context, signal_id=signal.id, statement="小学高年级家长群体存在学习管理退出困难"
    )
    opportunity = await commands.create_opportunity(
        repo, context, insight_id=insight.id, statement="学习责任转移计划"
    )
    ...
    concept = await commands.create_product_concept(
        repo, context, strategy_id=strategy.id, title="学习自主21天计划"
    )
...
async def _build_chain_with_ai_insight(repo, ai_context):
    ...
    signal = await commands.create_market_signal(
        repo, ai_context, raw_text="家长普遍反映每天辅导作业太累"
    )
    insight = await commands.create_customer_insight(
        repo,
        ai_context,
...
async def test_full_chain_fake_repository(fake_repo, human_context):
    signal, insight, opportunity, problem, hypothesis, strategy, concept = await _build_chain(
        fake_repo, human_context
    )

    chain = await queries.get_product_concept_chain(
        fake_repo, human_context, product_concept_id=concept.id
    )
