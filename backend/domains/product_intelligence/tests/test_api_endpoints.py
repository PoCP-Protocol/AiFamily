...
def test_full_chain_via_http(client):
    ...
    r = client.post(
        "/product-intelligence/customer-insights",
        json={"signal_id": signal_id, "statement": "insight"},
    )
    assert r.status_code == 200
    insight_id = r.json()["id"]

    r = client.post(
        "/product-intelligence/opportunities", json={"insight_id": insight_id, "statement": "opp"}
    )
    opp_id = r.json()["id"]

    r = client.post(
        "/product-intelligence/growth-problems", json={"opportunity_id": opp_id, "symptom": "sym"}
    )
    problem_id = r.json()["id"]

    r = client.post(
        "/product-intelligence/growth-hypotheses", json={"problem_id": problem_id, "statement": "h"}
    )
    assert r.json()["status"] == "DRAFT"
    hyp_id = r.json()["id"]

    r = client.post(
        "/product-intelligence/growth-strategies",
        json={"problem_id": problem_id, "hypothesis_ids": [hyp_id], "statement": "strat"},
    )
    strat_id = r.json()["id"]

    r = client.post(
        "/product-intelligence/product-concepts", json={"strategy_id": strat_id, "title": "concept"}
    )
...
def test_invalid_parent_returns_404(client):
    r = client.post(
        "/product-intelligence/customer-insights",
        json={"signal_id": "does-not-exist", "statement": "x"},
    )
...
def test_ai_actor_validate_rejected_human_accepted_via_http(client, context_slot):
    ...
    signal_id = r.json()["id"]
    r = client.post(
        "/product-intelligence/customer-insights", json={"signal_id": signal_id, "statement": "i"}
    )
    insight_id = r.json()["id"]
    r = client.post(
        "/product-intelligence/opportunities", json={"insight_id": insight_id, "statement": "o"}
    )
    opp_id = r.json()["id"]
    r = client.post(
        "/product-intelligence/growth-problems", json={"opportunity_id": opp_id, "symptom": "p"}
    )
    problem_id = r.json()["id"]
    r = client.post(
        "/product-intelligence/growth-hypotheses",
        json={"problem_id": problem_id, "statement": "h2"},
    )
    hyp_id = r.json()["id"]

    context_slot[0] = ActorContext(actor_id="ai-bot", actor_type="AI", tenant_scope="tenant-a")
    r = client.post(
        f"/product-intelligence/growth-hypotheses/{hyp_id}/validate",
        json={"reason": "auto-approve"},
    )
    ...
    r = client.post(
        f"/product-intelligence/growth-hypotheses/{hyp_id}/validate", json={"reason": "reviewed"}
    )
    assert r.status_code == 200
...
def test_cross_tenant_via_http_returns_404(client, context_slot):
    ...
    r = client.post(
        "/product-intelligence/customer-insights",
        json={"signal_id": signal_id, "statement": "tenant-b reaching into tenant-a"},
    )
    assert r.status_code == 404
...
def test_ai_actor_missing_provenance_via_http_returns_400(client, context_slot):
    ...
    r = client.post(
        "/product-intelligence/customer-insights",
        json={"signal_id": signal_id, "statement": "no provenance supplied"},
    )
