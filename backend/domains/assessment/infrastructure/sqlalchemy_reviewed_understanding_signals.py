...
Backs `assessment_reviewed_understanding_signals`
(`database/migrations/versions/0068_reviewed_understanding_signal.py`).
Only the `/assessment` scope kind has a producer wired in this codebase
today; `/problem-understanding` rows (bound by `understanding_run_ref`
instead of `assessment_session_id`) are representable by the schema and
this adapter's read/write paths, but this adapter deliberately does not
project a live draft-revocation/expiry status against a separate
problem-understanding snapshot table — no such table or producer exists
in this codebase yet. `effective_status`/`expires_at`/`revoked_at` as
written by `save_viewed_signal` are read back as-is.
"""
