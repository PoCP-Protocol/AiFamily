...
Scoped to the `/assessment` kind only: `family_understanding_draft_snapshots`
(the `/problem-understanding` scope's snapshot producer) has no table or
producer in this codebase yet — see `sqlalchemy_reviewed_understanding_signals`
module docstring. Cross-scope/expiry/revocation fail-closed behaviour for the
implemented `/assessment` kind is exercised here against a real Postgres
schema.
"""
...
async def test_guardian_view_is_durable_and_idempotent_across_sessions(session_factory) -> None:
    ...
    async with session_factory() as restarted_session:
        loaded = await SqlAlchemyReviewedUnderstandingSignals(
            restarted_session
        ).load_viewed_signal(
...
async def test_invalid_binding_is_rejected_before_writer(
    session_factory, changed, message
) -> None:
    command = replace(reviewed_command(), **changed)
    async with session_factory() as session:
        service = RecordReviewedUnderstandingService(
...
async def test_confirm_uses_exact_durable_binding_and_rejects_stale_version(
    ...
    async with session_factory() as restarted_session:
        ...
        with pytest.raises(
            AssessmentConflictError, match="understanding_signal_version_conflict"
        ):
