...
def test_self_help_failure_escalates_to_real_teacher_through_fgcn_human_gate() -> None:
    ...
    with TestClient(create_app()) as client:
        ...
        async def _seed_second_slot() -> None:
            ...
            offering = next(
                item for item in offerings if item.service_offering_ref == "COMMUNICATION"
            )
...
def test_ai_coach_sees_a_real_prior_course_completion_not_a_brand_new_family() -> None:
    ...
    with TestClient(create_app()) as client:
        ...
        snapshot = dev_wiring._journey_outcome_loop.snapshot(
            tenant_id=family_id, family_id=family_id
        )
        assert any(
...
def test_experience_signal_summary_aggregates_across_families_with_no_identity_leak() -> None:
    ...
    with TestClient(create_app()) as client:
        ...
        communication = next(
            ...
            if item["component_id"] == "COMMUNICATION"
        )
