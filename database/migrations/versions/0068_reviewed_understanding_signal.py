"""Persist the exact Model Gateway draft a guardian reviewed (Human Gate).

Ported from `origin/codex/integration-s1-mvp`'s `0005_reviewed_signal` +
`0006_understanding_scope_binding` migrations, consolidated into one
migration on top of this branch's current head. `assessment_session_id`
is nullable and paired with an optional `understanding_run_ref` so the
same table can later carry a `/problem-understanding` scoped signal (no
assessment session) without a schema change; only the `/assessment` scope
kind has a producer wired in this codebase today
(`reviewed_understanding_signals.RecordReviewedUnderstandingService` via
`sqlalchemy_reviewed_understanding_signals.SqlAlchemyReviewedUnderstandingSignals`).

Revision ID: 0068_reviewed_understanding_signal
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint(
            ...
            name="ck_reviewed_understanding_scope_binding",
        ),
    )
    op.create_index(
    ...
    op.create_index(
        "idx_reviewed_understanding_run_timeline",
        TABLE_NAME,
...
def downgrade() -> None:
    op.drop_index("idx_reviewed_understanding_run_timeline", table_name=TABLE_NAME)
